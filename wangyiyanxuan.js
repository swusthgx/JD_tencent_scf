...
async function doFairylandTask () {
    ...
    if ($.data) {
        for (let a = 0; a < $.data.data.length; a++) {
            //$.data.data[a].
            let remainChance =
                $.data.data[a].totalChance - $.data.data[a].finishedChance
    ...
    if ($.data) {
        for (let a = 0; a < $.data.length; a++) {
            if ($.data[a].canUpgrade == true) {
                ...
                console.log(
                    `'${$.data[a].buildingName}'升级需要金币${$.data[a].upgradePrice > 1000000
                        ? $.data[a].upgradePrice / 1000000 + 'M'
                        : $.data[a].upgradePrice / 1000 + 'K'
                    },去升级...`
                )
...
function rewardGoldCoin (links, buildingId) {
    //收取金币
    return new Promise((resolve) => {
        ...
        const options = {
            ...
            headers: {
                ...
                'User-Agent':
                    'Mozilla/5.0 (Linux; Android 10; ELS-AN00 Build/HUAWEIELS-AN00; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.93 Mobile Safari/537.36 yanxuan/6.7.9 device-id/3228a1a8086bea61a303e7f6305fcc1c app-chan-id/aos_market_huawei trustId/android_trustid_8821a74f57ec4f08bf2666e6bc82a51a',
    ...
    return new Promise((resolve) => {
        ...
        const options = {
            ...
            headers: {
                ...
                'Accept-Language': 'zh-CN,zh;q=0.9,en-CN;q=0.8,en-US;q=0.7,en;q=0.6',
                Referer:
                    'https://you.163.com/topic/v1/pub/msTR3Qv3wfpb.html?_stat_subject=23319',
                'Accept-Encoding': 'gzip, deflate',
                'Content-Length': 32,
                //   "Sec-Fetch-Site": "same-origin",
                //   "Sec-Fetch-Mode": "cors",
...
function fairylandReceiveReward (body) {
    //心愿城任务领取奖励
    return new Promise((resolve) => {
        let body1 = body
        const options = {
            body: JSON.stringify(body1),
            url:
                'https://act.you.163.com/act/napi/fairyland/receiveReward?csrf_token=' +
                $.usrToken,
            headers: {
                Cookie: $.cookie,
                Host: 'act.you.163.com',
...
function buildingUpgrade (body) {
    //心愿城升级建筑
    return new Promise((resolve) => {
        const options = {
            body: JSON.stringify(body),
            url:
                'https://act.you.163.com/act/napi/fairyland/buildingUpgrade?csrf_token=' +
                $.usrToken,
            headers: {
                Cookie: $.cookie,
                Host: 'act.you.163.com',
...
async function rewardCoin () {
    try {
        ...
        if ($.addCoin < 1000000) {
            ...
            $.message += `本次运行获得金币:${Math.floor(($.addCoin / 1000000) * 100) / 100
                }M\n`
            console.log(
                `本次运行获得金币:${Math.floor(($.addCoin / 1000000) * 100) / 100}M`
            )
        }
        if ($.goldCoinNum < 1000000) {
            ...
            $.message += `当前金币:${Math.floor(($.goldCoinNum / 1000000) * 10) / 10
                }M\n`
            console.log(
                `当前金币:${Math.floor(($.goldCoinNum / 1000000) * 10) / 10}M`
            )
        }
    ...
    return new Promise(async (resolve) => {
        const options = {
            ...
            headers: {
                ...
                'Accept-Language': 'zh-CN,zh;q=0.9,en-CN;q=0.8,en-US;q=0.7,en;q=0.6',
                Referer:
                    'https://m.you.163.com/sign-challenge/h5/index?appConfig=1_1_1',
                'Accept-Encoding': 'gzip, deflate',
            },
        }
        $.get(options, (err, resp, data) => {
...
function receiveSpecialGoldCoin () {
    //打开金币宝箱
    return new Promise((resolve) => {
        //const body = {"linkId":signLinkId,"serviceName":"dayDaySignGetRedEnvelopeSignService","business":1};
        const options = {
...
function watering () {
    ...
    return new Promise((resolve) => {
        //const body = {"linkId":signLinkId,"serviceName":"dayDaySignGetRedEnvelopeSignService","business":1};
        url =
            'https://act.you.163.com/act/napi/wish-tree/waterFertilization?csrf_token=1176afb489d52bd72f460b6d90400430'
            ...
            try {
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        data = JSON.parse(data)
                        console.log(
                            `浇水成功,剩余水滴${data.data.kettleWater}g, 明天可领取水滴${data.data.tomorrowWaterValue}g, 当前营养值${data.data.nutritionValue}`
                        )
                        $.data = data
                    }
...
function getUserTreeInfo () {
    ...
    return new Promise(async (resolve) => {
        url =
            'https://act.you.163.com/act/napi/wish-tree/getUserTreeInfo?csrf_token=1176afb489d52bd72f460b6d90400430&__timestamp=' +
            Date.now()
        const options = {
            url: url,
            headers: {
